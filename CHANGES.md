...
## 2026-08-06 (overnight) — Debug pass, research-backed prediction, new icon, email
**Debug/cleanup:** Removed 16 dead macOS duplicate (" 2") files. Full suite 165 tests pass. Audited every place that computes phase/cycle-day from a rigid cycle length; the calendar was the one causing wrong periods (fixed earlier) — others (weekly/plan projections) are forward-only and correct.
**Wearable correctness:** Verified the cycle guardian lets a freshly logged period override any older wearable ovulation (a stale ovulation can never override a new period). Wearable auto-fill only writes empty log fields; manual entries always win.
**Research-backed period prediction (Bull 2019 npj Digital Med; Li 2020/2023 npj Digital Med; Munro/FIGO 2018; Symul 2019; Broad 2022):** prediction now (1) uses the TYPICAL cycle length so short breakthrough bleeds don't bias the date early, (2) scales the window to the user's own variability and widens when irregular/thin history, (3) never predicts a period before ~21 days. Regression tests added (irregular history, logged-before-predicted, <3 cycles).
**New app icon:** brand crescent + gold disc on warm cream, matching the palette (dark brown #2c2820, gold #c8b89a). Generated for iOS (AppIcon), Android (adaptive), and PWA/web. Source in empower-react/icon-src/.
**Email:** app code already 100% on empowerhealthapp@gmail.com. Remaining personal-email routing is in external dashboards (Supabase/Netlify/GitHub/Apple) — see EMAIL_SETUP.md.
